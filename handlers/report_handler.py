...
async def process_author(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Referat necha bet bo'lishini tanlang:",
...
async def process_tier(callback: CallbackQuery, state: FSMContext):
    ...
    is_free = await check_free_usage(user_id, "referat")
    ...
    if is_free:
        await callback.message.edit_text(
            ...
            f"Mavzu: {data['topic']}\n"
            f"Sahifa: {tier} bet\n\n"
            "⏳ Referat tayyorlanmoqda, iltimos kuting...",
            parse_mode="HTML"
        )
        await mark_free_usage(user_id, "referat")
        await generate_and_send_report(callback.message, data, tier, user_id)
        await state.clear()
    else:
        import json
        order_id = f"ORDER_{uuid.uuid4().hex[:8].upper()}"
        params = json.dumps({"topic": data["topic"], "university": data["university"], "author": data["author"]})
        await create_order(order_id, user_id, "referat", tier, amount, params)
...
async def generate_and_send_report(message: Message, data: dict, tier: str, user_id: int):
    topic = data.get("topic")
    university = data.get("university")
    author = data.get("author")
    
    try:
        content = await generate_report(topic, tier)
        file_path = create_docx(content, f"Referat_{user_id}.docx", university, author, topic, "REFERAT")
